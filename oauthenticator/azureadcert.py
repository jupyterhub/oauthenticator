...
class AzureAdOAuthenticatorWithCertificate(AzureAdOAuthenticator):
    client_assertion_handler = Any(
        config=True, help="A callback that returns the client assertion in JWT form"
    )
    ...
    async def token_to_user(self, token_info):
        ...
        if (
            decoded['aud'] == self.client_id
            and decoded['tid'] == self.tenant_id
            and decoded['appidacr'] == '2'
            and decoded['appid'] == self.client_id
        ):
            return decoded
        else:
            return {}
    ...
    async def authenticate(self, handler, data=None, **kwargs):
        ...
        access_token_params = self.build_access_tokens_request_params_with_certificate(
            handler, data
        )
        # exchange the oauth code for an access token and get the JSON with info about it
        token_info = await self.get_token_info_with_certificate(
            handler, access_token_params
        )
