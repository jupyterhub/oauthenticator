...
c.AzureAdOAuthenticatorWithCertificate.oauth_callback_url = 'http://localhost:8000/hub/oauth_callback' 
...
try:
    ...
    secret_client = SecretClient(vault_url=f'https://{keyvault_name}.vault.azure.net/', credential=user_credential)
    ...
    b64sha1cert = base64.b64encode(public_certificate.fingerprint(hashes.SHA1())).decode()

    def jwthandler(handler, data):
        
        exp_time = datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(seconds=600)
